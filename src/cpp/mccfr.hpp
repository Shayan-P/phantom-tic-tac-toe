...
namespace mccfr {
    ...
    class RegretMinimizer {
        ...
        int dim;
    public:
        RegretMinimizer(int dim): dim(dim) {
            assert(dim <= MAX_DIM);
...
////////////////////////////////////////
    ...
    template<class Game>
    class MCCFR {
        static const T EXPLORATION = 0.6;
    ...
    private:
        T episode(ComputeMemo &memo, Game &state, Game::Player player, T reach_me=1.0, T reach_other=1.0, T reach_sample=1.0) {
            ...
            if(state.is_chance()) {
                ...
                episode(memo, state, player, reach_me, reach_other * p, reach_sample * p);
            }
            ...
            if(cur_player == player) {
                for(int i = 0; i < num_actions; i++) {
                    ...
                    memo.utility[i] *= reach_other;
                    memo.utility[i] /= reach_sample;
                }
